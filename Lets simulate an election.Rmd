...
combined_probs <- combined_probs %>%
  mutate(prob_winning = case_when((variable == "perdue_probs" & 
                                    wining_color == "win" ~ 
                                    length(which(perdue_probs > 0)) / n),
                                  (variable == "perdue_probs" & 
                                    wining_color == "lose" ~ 
                                    length(which(perdue_probs < 0)) / n),
                                  (variable == "ossof_probs" & 
                                    wining_color == "win" ~ 
                                    length(which(ossof_probs > 0)) / n),
                                  (variable == "ossof_probs" & 
                                    wining_color == "lose" ~ 
                                    length(which(ossof_probs < 0)) / n)))
...
ggplot((combined_probs %>% filter(variable == sprintf(candidate))), aes(value)) +
  ...
  labs(title = sprintf("Probability of %s", candidate)) +
...
```{r}
probability_winning_plot("ossof_probs")
```


```{r}
combined_probs %>% filter(wining_color == "win" & variable == "perdue_probs") %>% summarise(label_percent(accuracy = 0.01)(mean(prob_winning))) %>% select(-variable) %>% flatten_chr() 
```
