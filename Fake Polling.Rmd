...
```{r}
fake_polling <- function(distribution, size, max_spread, candidate) {
...
fake_polls <- as.data.frame(round(distribution(size, min = 0, max = max_spread), 1))
...
fake_boot_spread <- map(1:10000, ~sample(fake_polls$fake_spread, size = length(fake_polls), replace = TRUE)) %>%
  map_dbl(mean)
...
Perdue <- (mean(fake_boot_spread) * rnorm(size, mean(fake_boot_spread), 
...
Ossof <- (mean(fake_boot_spread) * rnorm(size, mean(fake_boot_spread), 
...
```{r}
fake_polling(runif, 10000, 3, "Ossof")
```
...
Perdue <- (mean(fake_boot_spread) * rnorm(100, mean(fake_boot_spread), 
                                                     sd(fake_boot_spread))) + empty_vec 
